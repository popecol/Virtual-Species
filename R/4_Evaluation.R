...
plot.gam(sim_fit, scale = 0, scheme = 2, pages = 1, seWithMean = TRUE, residuals = FALSE)
par(op)
...
yrs <- seq(min(years), max(years), 1)
nd <- data.frame(year = yrs, fyear = factor(yrs), plot_id = levels(ve$plot_id)[1])
pred <- predict(ft, nd, exclude = c("s(plot_id)"), type = "link", se = TRUE)
pre <- data.frame(year = yrs, fit = pred$fit, se = pred$se.fit)
...
polygon(c(yrs, rev(yrs)), c(pre$lci, rev(pre$uci)), border = NA, col = col4[2])
lines(fit ~ yrs, pre, col = col[2], lty = 1, lwd = 2)
lines(yrs, true_d, col = col[1], lwd = 2, lty = 2)
